...
categorical_vars <- c("TRPTRANS", "HHFAMINC_new", "URBRUR", "HH_RACE_new",
                      "EDUC", "R_SEX", "PHYACT", "W_CANE", "W_WLKR",
                      "W_CRUTCH", "W_SCOOTR", "W_CHAIR", "CONDTRAV",
                      "CONDRIDE", "CONDNIGH", "CONDRIVE", "CONDPUB", "CONDSPEC")
...
# Manually fixing a problematic variable
trip_data_merged_copy$TRPTRANS <- factor(trip_data_merged_copy$TRPTRANS, levels = levels(trip_data_downsampled$TRPTRANS))
trip_data_merged_copy$TRPTRANS <- droplevels(trip_data_merged_copy$TRPTRANS)
...
### Comparison of model 4 vs. model 5 (SMOTE) vs. model 6 (downsampling)
...
trip_data_downsampled$TRPTRANS <- as.numeric(as.factor(trip_data_downsampled$TRPTRANS))
trip_data_downsampled$URBRUR <- as.numeric(as.factor(trip_data_downsampled$URBRUR))
trip_data_downsampled$PHYACT <- as.numeric(as.factor(trip_data_downsampled$PHYACT))
trip_data_downsampled$HHFAMINC_new <- as.numeric(as.factor(trip_data_downsampled$HHFAMINC_new))
trip_data_downsampled$EDUC <- as.numeric(as.factor(trip_data_downsampled$EDUC))
```
...
model7 <- gam(medical_trips_response ~ s(R_AGE, bs = "cs") + s(TRVLCMIN, bs = "cs") + s(DWELTIME, bs = "cs") + TRPTRANS + HHSIZE + HHFAMINC_new + WRKCOUNT + URBRUR + HH_RACE_new + EDUC + R_SEX + PHYACT + W_CANE + W_WLKR + W_CRUTCH + W_SCOOTR + W_CHAIR + CONDTRAV + CONDRIDE + CONDNIGH + CONDRIVE + CONDPUB + CONDSPEC + ti(TRPTRANS, by = URBRUR)  + ti(R_AGE, DWELTIME) + ti(TRVLCMIN, TRPTRANS), family = binomial(link = "logit"), data = trip_data_downsampled)  
...
trip_data_merged_copy$TRPTRANS <- as.numeric(as.factor(trip_data_merged_copy$TRPTRANS))
trip_data_merged_copy$URBRUR <- as.numeric(as.factor(trip_data_merged_copy$URBRUR))
trip_data_merged_copy$PHYACT <- as.numeric(as.factor(trip_data_merged_copy$PHYACT))
trip_data_merged_copy$HHFAMINC_new <- as.numeric(as.factor(trip_data_merged_copy$HHFAMINC_new))
trip_data_merged_copy$EDUC <- as.numeric(as.factor(trip_data_merged_copy$EDUC))
```
